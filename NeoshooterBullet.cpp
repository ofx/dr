...
NeoshooterBullet::NeoshooterBullet(Player *owner) : Bullet(owner)
{
    this->m_Speed = 2.0f;
...
NeoshooterBullet::~NeoshooterBullet(void)
{
    // Delete the sprite
...
void NeoshooterBullet::Render(float dt)
{
    // Render the bullet
    this->m_Sprite->Render(this->m_Position.x, this->m_Position.y);
}

void NeoshooterBullet::Update(float dt)
{
    Bullet::Update(dt);
    ...
    // Update the position
    this->m_Position = cpVect(cpBodyGetPos(this->m_Body));
}